...
/**
 * Navigate to the Fax panel and pre-select this case.
 * Works from both kanban card icon and modal Send Fax button.
 */
function openFaxForCase(caseId) {
  // Close any open modal first
  closeCaseModal();
  if (typeof switchPanel === "function") switchPanel("fax");
  // After panel renders, pre-select the case
  setTimeout(() => {
    if (typeof prefillFaxFromCase === "function") prefillFaxFromCase(caseId);
  }, 120);
}

// openFaxModal kept as alias for backward compat — now routes to panel
function openFaxModal(caseId) { openFaxForCase(caseId); }
